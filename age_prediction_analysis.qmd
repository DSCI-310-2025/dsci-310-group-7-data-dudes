...
- **broom**: To tidy up model outputs for easier interpretation [@broom].  
- **caret**: For creating machine learning models and cross-validation [@caret].  
- **dplyr**: For data manipulation tasks like filtering and summarizing [@dplyr].  
- **docopt**: For parsing command-line arguments [@docopt].  
- **ggplot2**: For data visualization and creating plots [@ggplot2].  
- **readr**: For reading data files into R [@readr].  
- **rmarkdown**: For creating dynamic reports [@rmarkdown].  
- **tidymodels**: For building machine learning workflows [@tidymodels].  
- **tidyr**: For reshaping and tidying data [@tidyr].

You can obtain the complete citation information for each of these packages by running `citation("package_name")` in R.
