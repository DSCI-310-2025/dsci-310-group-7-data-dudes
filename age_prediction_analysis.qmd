...
head(data, n=c(18, 6))
```
We can see there is heterogeneity in column types, with most being `<dbl>` but some as `<chr>`.
To make this a classification problem, we will create a Class variable, either being Youth (21 & Under) or Adult (22 & Over).
All other columns should be numeric.
We don't need `age` or `n` so we will remove these variables.
```{r}
data_clean <- data %>%
  mutate(
